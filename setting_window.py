    ...
    def go_to_detection(self):
        if self.location_input.text() == '' or self.sendTo_input.text() == '':
            ...
            if self.detection_window is None:
                self.detection_window = DetectionWindow()  # Create an instance of DetectionWindow if it's not already created
            if self.detection_window.isVisible():
                print('Detection window is already open!')
            else:
                self.detection_window.create_detection_instance(self.token, self.location_input.text(),
